<template>
  <div v-if="bookExist">
    <div v-if="requestsExist">
      <h1 v-if="loaded">Requests of Book - {{ this.book_name }}</h1>
      <button 
        @click="showActive" 
    ...
    <div v-else>
      <div v-if="loaded">
        <h2>No Available Requests of this book!</h2>
      </div>
    </div>
  </div>
   <div v-else>
      ...
      methods: {
        async loadRequests() {
          try {
            ...
            const response = await axios.get(`http://127.0.0.1:5000/api/requests?book_id=${this.id}`, {
              headers: {
            ...
            if(this.active_count!=0 || this.inactive_count!=0){
              ...
              if(this.active_count!=0){
                this.book_name=this.active_requests[0]['book_name'];
              } else{
                this.book_name=this.inactive_requests[0]['book_name'];
              }
            }

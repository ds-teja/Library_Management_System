...
data() {
    return {
        ...
        chartOptions: {
            ...
            title: {
                display: true,
                text: 'Popular Sections for the Past 6 Months'
            }
            }
        }
    };
...
methods: {
    fetchPopularSections() {
        ...
        axios.get(`http://127.0.0.1:5000/librarian/stats/sections`,{
                    headers:{
                        'Authorization': `Bearer ${access_token}`
                    }
                }).then((response) => {
        const sections = response.data;
        this.chartData.labels = sections.map((section) => section.section);
